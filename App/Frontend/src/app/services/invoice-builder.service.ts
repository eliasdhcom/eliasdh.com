...
import { Customer, CustomerLocation, SubscriptionFrequency } from './customers.service';
import { InvoiceStatus } from './invoices.service';
...
export class InvoiceBuilderService {
    ...
    buildInvoices(customers: Customer[], allStatuses: InvoiceStatus[]): Invoice[] {
        ...
        let invoices = this.generateRawInvoices(customers);
        ...
        invoices.forEach(inv => {
            ...
            if (stored) {
                ...
                if (stored.paid) {
                    ...
                    if (stored.frequency)                  inv.frequency        = stored.frequency as SubscriptionFrequency;
        ...
        const paidRecordByBase = new Map<string, InvoiceStatus>(
            allStatuses
                .filter(s => s.paid)
                .map(s => [`${s.customerId}_${s.subscriptionId}_${s.periodStart}_${s.invoiceType}`, s])
        );
    ...
    periodEndFromFreq(start: Date, freq: string): Date {
    ...
    private generateRawInvoices(customers: Customer[]): Invoice[] {
    ...
    getBillingPeriods(startDateStr: string, frequency: SubscriptionFrequency, endDate: Date): { start: Date; end: Date }[] {
