...
async function createUser(data) {
    ...
    const result = await getDb().execute({
        sql:  `INSERT INTO users (email, password_hash, first_name, last_name, role, company, phone, birth_date, avatar, active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
        args: [
            data.email,       hash,
            data.firstName  ?? '',
            data.lastName   ?? '',
...
module.exports = { getAllUsers, getUserById, setActive, updateUser, createUser, deleteUser };